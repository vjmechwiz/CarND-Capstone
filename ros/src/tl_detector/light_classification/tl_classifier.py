...
class TLClassifier(object):
    ...
    def get_classification(self, image):
        ...
        with tf.Session() as sess:
            saver = tf.train.import_meta_graph(self.trained_model_graph)
            saver.restore(sess, tf.train.latest_checkpoint(self.trained_model_folder + '.'))
            graph = tf.get_default_graph()
            x = graph.get_tensor_by_name("X:0")
            keep_prob = graph.get_tensor_by_name("Keep_prob:0")
            logits = graph.get_tensor_by_name("Logits:0")
            test_prediction = sess.run(tf.argmax(logits, 1), feed_dict={x: test_input, keep_prob: 1.0})
